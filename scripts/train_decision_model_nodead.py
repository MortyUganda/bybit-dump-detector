"""
Эксперимент: decision-модель БЕЗ мёртвых фичей.

Копия train_decision_model.py с одним отличием — фичи из DEAD_FEATURES
удаляются из обучающего набора. На запуске от 2026-05-10 у этих фичей
importance=0 (LightGBM их вообще не использует) либо очень низкое:
  - spread_pct          (есть spread_pct_z)
  - bid_depth_change_5m (есть bid_depth_change_5m_z)
  - session             (категориальная без encoding; hour_of_day покрывает)
  - is_weekend          (мало weekend'ов)
  - f_momentum_loss     (importance≈13)
  - f_cvd_divergence    (importance≈62)

Цель — проверить гипотезу: уберём шум → модель чище разделяет классы
→ AUC может вырасти на 0.005–0.010.

Модель сохраняется как decision_model_nodead_*.pkl и НЕ перезаписывает
prod-манифест decision_model_features.json — чтобы случайно не подменить
рабочую модель.
"""
...
# Фичи, которые на последнем прогоне имели importance=0 или очень низкий gain.
# Удаляются из обучения целиком (и из COMMON_FEATURES, и из engineered).
DEAD_FEATURES = {
    "spread_pct",            # 0.0  — есть z-вариант
    "bid_depth_change_5m",   # 0.0  — есть z-вариант
    "session",               # 0.0  — категориальная без encoding
    "is_weekend",            # 0.0  — мало weekend'ов
    "f_momentum_loss",       # ~13  — почти шум
    "f_cvd_divergence",      # ~62  — слабая
}
...
def merge_datasets(
    ...
    feature_cols = [
        ...
        if all(c in d.columns for d in all_dfs) and c not in DEAD_FEATURES
    ]
    print(f"\nОбщих фичей: {len(feature_cols)} (исключено DEAD: "
          f"{sorted(DEAD_FEATURES & set(COMMON_FEATURES))})")
    ...
    for f in GROUP1_FEATURES + GROUP2_FEATURES:
        if f in df.columns and f not in feature_cols and f not in DEAD_FEATURES:
...
def main() -> None:
    ...
    model_path = model_dir / f"decision_model_nodead_{date.today()}_{current_time}.pkl"
    ...
    # ВНИМАНИЕ: prod-манифест decision_model_features.json НЕ перезаписываем —
    # это эксперимент. Сохраняем только timestamp-манифест рядом с pkl.
    manifest_ts = model_dir / f"decision_model_nodead_{date.today()}_{current_time}_features.json"
    with open(manifest_ts, "w", encoding="utf-8") as f:
        json.dump(manifest_data, f, ensure_ascii=False, indent=2)

    print(f"✅ Манифест фичей сохранён: {manifest_ts} (фичей: {len(trained_features)})")
    print("⚠️  prod-манифест decision_model_features.json НЕ изменён (эксперимент)")
